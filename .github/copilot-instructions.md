...
For branch naming, commit messages, pull request titles, and pre-push validation, follow `.github/instructions/git-conventions.instructions.md` (**Branch Naming**, **Pull Requests**, pre-push where applicable).

**Pull requests:** When you open a PR for a GitHub issue, the **PR title must reference that issue** the same way as commit messages. Use the **Pull Requests** section in `.github/instructions/git-conventions.instructions.md` for pattern, regex, and examples (typically `GH-<issue-number>: <short description>` with a space after the colon). Do not use a title with no `GH-<n>:` prefix; non-compliant titles are rejected by repository rules. The issue number must be the one **this PR closes** (see **Which issue number (single source of truth)** in that section)—not a related or linked issue number picked up from the body or comments.

**Lockfiles — never commit install drift (ALL PRs):** Install dependencies with **`npm ci`**, never `npm install`, unless you are deliberately adding or removing a package. `npm install` rewrites `package-lock.json` whenever your npm version differs from the one that generated it, producing large metadata-only diffs. **A PR must not modify `package-lock.json` (or `yarn.lock`) unless it also modifies the matching `package.json`.** Before opening or updating any PR, run `git diff --name-only origin/master...HEAD`. Revert a lockfile with `git checkout origin/master -- <path>/package-lock.json` **only when its matching `package.json` is unchanged**. If you added, removed, or upgraded a dependency, keep the regenerated lockfile and commit it with that `package.json` — restoring from `origin/master` would leave them out of sync. Note `git status` looks clean if the lockfile is already committed, so diff against the base branch. See **Lockfiles** in `.github/instructions/git-conventions.instructions.md`.
