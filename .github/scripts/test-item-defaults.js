...
async function setAssignee(issue) {
    ...
    const shouldAct = existing.length === 0 || (existing.length === 1 && existing[0] === DEFAULT_CODEOWNER);

    if (!shouldAct) {
        console.log(`Assignee already set to [${existing.join(', ')}] — leaving alone.`);
        return;
    }
    ...
    if (ISSUE_CREATOR === DEFAULT_CODEOWNER) {
        console.log(`Issue creator is the default codeowner (${DEFAULT_CODEOWNER}) — leaving assignee alone.`);
        return;
    }
...
async function setMilestone(issue) {
    if (issue.milestone) {
        console.log(`Milestone already set to "${issue.milestone.title}" — leaving alone.`);
        return;
    }
...
async function getProjectAndAddIssue(issueNodeId) {
    console.log(`Fetching project ${PROJECT_ORG}/${PROJECT_NUMBER} and its fields...`);
...
async function setSingleSelectField(project, itemId, fieldName, optionName, currentFieldValues = {}) {
    if (currentFieldValues[fieldName]) {
        console.log(`${fieldName} already set to "${currentFieldValues[fieldName]}" — leaving alone.`);
        return;
    }

    const field = project.fields.nodes.find(f => f.name === fieldName && f.options);
    if (!field) {
    ...
    if (!option) {
        console.warn(
            `Option "${optionName}" not found in field "${fieldName}" — available options: ${field.options
                .map(o => o.name)
                .join(', ')}. Skipping.`
        );
        return;
...
async function setCurrentSprint(project, itemId, currentFieldValues = {}) {
    ...
    if (!currentIteration) {
        ...
        if (futureIterations.length > 0) {
            console.log(`No active sprint today — using next upcoming sprint: "${futureIterations[0].title}"`);
            await setIterationValue(project, itemId, iterationField, futureIterations[0]);
            return;
        }
    ...
    console.log(`Setting sprint to current iteration: "${currentIteration.title}"`);
...
function parseParentIssueNumber(body) {
    ...
    const match = body.match(/###\s*Parent\s*Issue\s*\n\n\s*(.+)/i);
    ...
    console.warn(`Could not parse parent issue number from: "${value}" — skipping.`);
    return null;
}
...
async function linkParentIssue(issue) {
    ...
    console.log(`Linking issue #${ISSUE_NUMBER} as sub-issue of #${parentNumber}...`);
    try {
        ...
        console.warn(`Could not link parent issue #${parentNumber}: ${error.message}`);
    }
}
...
(async () => {
    try {
        ...
        const { project, itemId, currentFieldValues } = await getProjectAndAddIssue(issue.node_id);

        // Step 4: Set Status to "To Do"
        await setSingleSelectField(project, itemId, 'Status', STATUS_DEFAULT, currentFieldValues);
