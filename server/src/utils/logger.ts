  ...
  if (isArray(obj)) {
    ...
    Object.keys(obj).forEach((key) => {
      if (SECRET_WORDS.includes(key)) {
        ...
        isPlainObject(obj[key as keyof typeof obj]) ||
        isArray(obj[key as keyof typeof obj])
      ) {
    ...
    if (appender.type === "console" || appender.type === "file") {
      if (isPatternLayout(appender.layout)) {
        ...
        patternLayout.tokens = {
          ...
          accountId: () =>
            getAsyncLocalStorageResource<string>(ACCOUNT_ID) || "unknown",
...
export default function getLogger(
  category: "server" | "got" | "simulator" | "access" = "server"
) {
