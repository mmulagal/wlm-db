...
import { GetAmiSchema, GetVpcsListSchema, GetAdsSchema, GetFSxRegionsSchema } from './schemas/aws-schemas';
...
export default function awsRoutes(fastify: FastifyInstance) {
    ...
    server.get(`${API_PREFIX_PATH}/fsx/regions`, { schema: GetFSxRegionsSchema }, async (request, reply) => {
        const {
            params: { credentialsId, region }
        } = request;
