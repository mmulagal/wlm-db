...
function publishScanStatus(msg: ScanStatusMessage): void {
    ...
    logger.info('WAD: published scan status', { taskId: msg.taskId, status: msg.status });
}

function publishScanResult(msg: WadScanResultRecord): void {
    publish(WAD_SCAN_RESULTS_QUEUE, msg);
    logger.info('WAD: published scan result', { taskId: msg.taskId, configurationId: msg.configurationId });
}
