...
def main():
    ...
    try:
        ...
        if not volume_uuids and not lun_uuids:
            ...
            log_error("  databases residing on FSxN storage.")
            log_error("  Alternatively, for CDB instances, none of the PDBs")
            log_error("  may be in READ WRITE mode")
            log_error("")
            log_error("Other possible causes:")
            log_error("  - StorageManagementAddress specified is not reachable.")
            log_error("  - Oracle files are on non-FSxN storage backend.")
