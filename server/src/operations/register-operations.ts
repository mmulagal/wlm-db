...
import { attempt, cloneDeep, compact, isEmpty, uniqBy, isArray } from 'lodash-es';
...
async function registerOracleInstance(
    ...
    try {
        ...
        await installJq(accountId, region, credentialsId, ec2InstanceId, hostJobId);
...
async function installJq(
    ...
    logger.info('Check and install jq', { accountId, region, credentialsId, instanceId });
    ...
    const { id: childJobId } = await registerJob(accountId, credentialsId, region, {
        ...
        resourceName: accountId,
        parentJobId,
        name: `Check and install jq on instance ${instanceId}`,
        startTime: Date.now(),
        description: `Check and install jq on instance ${instanceId}`
    });
    ...
    try {
        const ssmresponse = await callSsmExecution({
            credentialsId,
            region,
            commands: [installJqOnLinuxHost(signedUrls)],
            ec2InstanceId: instanceId,
            comment: 'Check and install jq on linux host',
            accountId,
            documentName: SSM_RUN_SHELL_SCRIPT_DOC,
            documentVersion: SSM_RUN_SHELL_SCRIPT_DOC_VERSION
        });
        ...
        if (!parsedResponse) {
            throw new Error(`Failed to check/install jq. Reason: ${cleanResponse}`);
        }

        if (parsedResponse?.installationSuccessful !== 'true') {
            throw new Error(`Failed to install jq. ${parsedResponse?.error ?? ''}`);
        }

        status = JOBSTATUS.COMPLETED;
    } catch (error: any) {
        logger.error('Error while installing jq', {
            accountId,
            region,
            credentialsId,
            instanceId,
    ...
    if (isReplicaInfoRequired) {
        try {
            ...
            if (instancesWithAoag && instancesWithAoag.length > 0) {
                ...
                // Get current instance name from aoagIpToEc2Details for FCI mapping
                let currentInstanceName = '';
                for (const [, ec2] of aoagIpToEc2Details.entries()) {
                    if (ec2.instanceId === instanceId) {
                        currentInstanceName = ec2.instanceName;
                        break;
                    }
                }
                        ...
                        for (const { agName, primaryReplica, replicas } of availabilityGroups) {
                            if (replicas && isArray(replicas)) {
                                for (const { role: replicaRole, replica: replicaName, isLocalReplica } of replicas) {
                                    ...
                                    if (ec2InstanceId) {
                                        const isDuplicate = replicaInfoObject.some(
