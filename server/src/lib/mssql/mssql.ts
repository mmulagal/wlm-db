...
async function getDBSummary(credentialsId:string, region:string, instanceId:string, offset:number, rowscount: number){
    const params = {
        DocumentName:DOCUMENTNAME,
        InstanceIds:[instanceId],
        Parameters:{
            commands:[
                `${PSSCRIPT} -Query "${DATABASES(offset, rowscount)}"`
            ],
        },
    };
