 ...
 */
export interface JsonValidator<T extends object> {
    /**
 ...
 * @returns A `JsonValidator<T>` instance.
 */
export function createJsonValidator<T extends object = object>(schema: string, typeName: string): JsonValidator<T> {
    const options = {
