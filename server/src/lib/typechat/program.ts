...
import { JsonTranslator, createJsonTranslator } from './translator';
 ...
 * references can be nested in objects and arrays.
 */
type JsonValue = string | number | boolean | null | { [x: string]: Expression } | Expression[];
 ...
 */
function createModuleTextFromProgram(jsonObject: object): Result<string> {
    const steps = (jsonObject as Program)['@steps'];
    if (
        !(
            Array.isArray(steps) &&
            steps.every(
                step => typeof step === 'object' && step !== null && Object.prototype.hasOwnProperty.call(step, '@func')
            )
        )
    ) {
 ...
 */
function createProgramTranslator(model: LanguageModel, schema: string): JsonTranslator<Program> {
    ...
    function createRequestPrompt(request?: string) {
        return (
            ...
            `"""\n${request}\n"""\n` +
            'translate it into a JSON object with 2 spaces of indentation and no properties with the value undefined'
            // `You need to take the user input and if the user intent matches any of the definitions above, then response accordingly`
        );
    }
