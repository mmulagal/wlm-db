...
import { getMissingPermissionsList } from '../../../src/lib/aws/iam';
...
describe('List permissions required', () => {
    it('list permissions required to deploy the stack - with all resources', async () => {
        const { permissions } = await getMissingPermissionsList(CREDENTIALS_ID, REGION);
        expect(permissions).toBeDefined();
    });

    it('list permissions required to deploy the stack - with skipped resources', async () => {
        const { permissions } = await getMissingPermissionsList(CREDENTIALS_ID, REGION, [SNS]);
