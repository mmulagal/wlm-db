...
let RESOURCE_ID = '6cbdabbfe3fb147e';
let CREDENTIALS_ID = DEFAULT_AWS_CREDENTIALS_ID;
beforeAll(async () => {
...
describe(
    'Continuous optimization optimize operations',
    () => {
        ...
        it('Optimize sizing parameters', async () => {
            const response = await optimizeSizing(ACCOUNT_ID, CREDENTIALS_ID, DEFAULT_AWS_REGION, RESOURCE_ID, 'f4b7c5d3-e1f6-4g2a-9b5d', `${OPTIMIZE_SIZING_CONFIGS.HEADROOM, OPTIMIZE_SIZING_CONFIGS.LOG_DRIVE_SIZE}`);
        ...
        it('Optimize sizing parameters - invalid type', async () => {
            try {
                await optimizeSizing(ACCOUNT_ID, CREDENTIALS_ID, DEFAULT_AWS_REGION, RESOURCE_ID, 'f4b7c5d3-e1f6-4g2a-9b5d', 'invalid-type');
            } catch (error) {
