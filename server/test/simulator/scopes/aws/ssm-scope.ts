...
const diskSizeParams = {
    commands: [
        'sqlcmd -S "$env:computername" -C -Q "SET NOCOUNT ON; SELECT CAST(SUM(CAST(size AS bigint)) * 8 * 1024 AS bigint) AS TotalSize FROM sys.master_files FOR JSON PATH" -y 0'
    ]
};

const diskDataParams = {
    commands: [
        'sqlcmd -S "$env:computername" -C -Q "SET NOCOUNT ON; WITH presel AS (SELECT database_id, FILE_ID,LEFT(mf1.physical_name,3) AS Volume, ROW_NUMBER() OVER (PARTITION BY LEFT(mf1.physical_name,3) ORDER BY mf1.database_id) AS RowNum\n                                FROM sys.master_files mf1)\n                                ,roundtwo AS (SELECT DISTINCT pr.database_id, pr.FILE_ID\n                                FROM presel pr\n                                WHERE pr.RowNum = 1)\n                                SELECT ovs.total_bytes AS total, ovs.available_bytes AS remaining\n                                FROM roundtwo mf\n                                CROSS APPLY sys.dm_os_volume_stats(mf.database_id, mf.FILE_ID) ovs FOR JSON PATH" -y 0'
    ]
};
const serGUIDParams = {
    commands: [
...
const getInstanceGuidCommand = { commands: [`sqlcmd -S "$env:computername" -C -Q "${INSTANCE_GUID}" -y 0`] };
...
ssmMock
    ...
    .on(SendCommandCommand, params => params.Comment === 'Get mount details for Oracle db')
...
ssmMock
    ...
    .resolves(
        getSampleCommandResponseWithOutput(
            ...
            '{"oradbsan":{"isCDB":false,"isASMManaged":false,"mountDetails":{"REDO_LOGS":[{"isAsmManaged":false,"mountIP":"10.0.1.10","mountPoint":"/wlmdb_oracleredo","protocol":"NFS","copiesCount":1}],"ARCHIVE_LOGS":[{"isAsmManaged":false,"mountIP":"10.0.1.10","mountPoint":"/wlmdb_oraclearch","protocol":"NFS","copiesCount":1}],"CONTROL_FILES":[{"isAsmManaged":false,"mountIP":"10.0.1.10","mountPoint":"/wlmdb_oraclectrl","protocol":"NFS","copiesCount":1}],"TEMP_FILES":[{"isAsmManaged":false,"mountIP":"10.0.1.10","mountPoint":"/wlmdb_oracletemp","protocol":"NFS","copiesCount":1}],"DATA_FILES":[{"isAsmManaged":false,"mountIP":"10.0.1.10","mountPoint":"/wlmdb_oracledata","protocol":"NFS","copiesCount":1}],"FRA":[]}}}'
        )
    )
    .on(GetCommandInvocationCommand, {
