...
BEGIN
  ...
  FOR r IN (
    ...
    SELECT
      ...
      ROUND(db_time_seconds / interval_seconds, 2) as active_sessions,
      -- Peak values = averages (STATSPACK has no sub-interval peak tracking)
      ...
      ROUND(db_time_seconds / interval_seconds, 2) as peak_active_sessions
    FROM snap_metrics
...
SPOOL OFF
SET TERMOUT ON
PROMPT || STATSPACK data collection complete (hybrid format). Output file: &file_name
