...
try {
    ...
    $TargetPortalAddresses = Invoke-WithRetry -Command { (Get-FSXStorageVirtualMachine | ? { $_.FileSystemId -eq $fslist.FileSystemId -And $_.Name -eq $SQLVMName }).Endpoints.Iscsi.IpAddresses }
    $token = Invoke-RestMethod -Headers @{"X-aws-ec2-metadata-token-ttl-seconds" = "21600" } -Method PUT -Uri "http://169.254.169.254/latest/api/token"
