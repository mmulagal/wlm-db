...
try {
    ...
    elseif ($NodeType -eq 'primary') {
        ...
        $FCIConfigure = @(
            ...
            @{Command = "C:\\cfn\\scripts\\common\\SetMaxDOP.ps1 -DomainAdminUser `"$DomainAdminUser`" -Parentstackname `"$DeploymentName`" -NetBIOSName `"$SqlFsxServerNetBiosName`" -ClusterName `"$SqlFSxWSFCName`""; UseExecutionPolicy = $false },
