...
DECLARE
  ...
  BEGIN
    ...
    -- Use TO_CHAR with format that ensures leading zero
...
BEGIN
  ...
  BEGIN
    SELECT DECODE(count(*), 0, 'false', 'true') INTO v_is_dataguard FROM v$dataguard_config;
  ...
  FOR r IN (
    ...
    -- Add standbyHosts array if DataGuard is enabled
    DBMS_OUTPUT.PUT('"standbyHosts": [');
    IF v_is_dataguard = 'true' THEN
      ...
      BEGIN
        OPEN standby_cur FOR
          'SELECT DISTINCT destination FROM v$archive_dest_status WHERE status = ''VALID'' AND dest_id > 1';
        LOOP
          FETCH standby_cur INTO v_standby_dest;
          ...
          ELSE
            DBMS_OUTPUT.PUT(', ');
          END IF;
          DBMS_OUTPUT.PUT(escape_json(v_standby_dest));
        END LOOP;
        CLOSE standby_cur;
      EXCEPTION WHEN OTHERS THEN NULL;
      END;
        ...
        BEGIN
          FOR pdb IN (
            ...
            ELSE
              DBMS_OUTPUT.PUT(', ');
            END IF;
            DBMS_OUTPUT.PUT(escape_json(pdb.name));
  ...
  FOR r IN (
    WITH 
    -- Shared CTE pipeline: average rates from cumulative stats + peak rates from sysmetric
    awr_data AS (
      SELECT
        s.snap_id,
        s.instance_number,
        ...
        LAG(st.value) OVER (PARTITION BY st.stat_name, st.instance_number ORDER BY s.snap_id) as prev_value
    ...
    delta_data AS (
      SELECT
        ...
        end_interval_time,
        stat_name,
        current_value - prev_value as delta_value
      FROM awr_data
    ...
    avg_metrics AS (
      SELECT
        ...
        SUM(CASE WHEN stat_name = 'physical read total IO requests' THEN delta_value ELSE 0 END) as read_io_requests,
        SUM(CASE WHEN stat_name = 'physical write total IO requests' THEN delta_value ELSE 0 END) as write_io_requests,
        SUM(CASE WHEN stat_name = 'physical read total bytes' THEN delta_value ELSE 0 END) as read_bytes,
        SUM(CASE WHEN stat_name = 'physical write total bytes' THEN delta_value ELSE 0 END) as write_bytes,
        -- DB time is in microseconds; divide by 1,000,000 for seconds.
        -- For idle/light workloads, this may produce sub-second values that ROUND to 0.
        SUM(CASE WHEN stat_name = 'DB time' THEN delta_value ELSE 0 END) / 1000000 as db_time_seconds
      FROM delta_data
      GROUP BY snap_id, instance_number, begin_interval_time, end_interval_time
      HAVING EXTRACT(DAY FROM (end_interval_time - begin_interval_time)) * 86400 +
             EXTRACT(HOUR FROM (end_interval_time - begin_interval_time)) * 3600 +
             EXTRACT(MINUTE FROM (end_interval_time - begin_interval_time)) * 60 +
             EXTRACT(SECOND FROM (end_interval_time - begin_interval_time)) > 0
    ),
    rate_metrics AS (
      SELECT
        snap_id,
        instance_number,
        interval_seconds,
        read_io_requests / interval_seconds as avg_read_iops,
        write_io_requests / interval_seconds as avg_write_iops,
        (read_io_requests + write_io_requests) / interval_seconds as avg_total_iops,
    ...
    peak_metrics AS (
      SELECT
        snap_id,
        instance_number,
        MAX(CASE WHEN metric_name = 'Physical Read Total IO Requests Per Sec' THEN maxval ELSE 0 END) as peak_read_iops,
        MAX(CASE WHEN metric_name = 'Physical Write Total IO Requests Per Sec' THEN maxval ELSE 0 END) as peak_write_iops,
        MAX(CASE WHEN metric_name = 'Physical Read Total Bytes Per Sec' THEN maxval / 1024 / 1024 ELSE 0 END) as peak_read_mbps,
        MAX(CASE WHEN metric_name = 'Physical Write Total Bytes Per Sec' THEN maxval / 1024 / 1024 ELSE 0 END) as peak_write_mbps,
        MAX(CASE WHEN metric_name = 'Average Active Sessions' THEN maxval ELSE 0 END) as peak_active_sessions
      FROM dba_hist_sysmetric_summary
      WHERE dbid = v_dbid
        AND begin_time > SYSDATE - c_lookback_days
        AND metric_name IN (
    ...
    combined_metrics AS (
      SELECT
        ...
        r.avg_write_mbps,
        r.avg_total_mbps,
        r.avg_active_sessions,
        NVL(p.peak_read_iops, r.avg_read_iops) as peak_read_iops,
        NVL(p.peak_write_iops, r.avg_write_iops) as peak_write_iops,
        NVL(p.peak_read_iops, r.avg_read_iops) + NVL(p.peak_write_iops, r.avg_write_iops) as peak_total_iops,
        NVL(p.peak_read_mbps, r.avg_read_mbps) as peak_read_mbps,
        NVL(p.peak_write_mbps, r.avg_write_mbps) as peak_write_mbps,
        NVL(p.peak_read_mbps, r.avg_read_mbps) + NVL(p.peak_write_mbps, r.avg_write_mbps) as peak_total_mbps,
        NVL(p.peak_active_sessions, r.avg_active_sessions) as peak_active_sessions
      FROM rate_metrics r
      LEFT JOIN peak_metrics p ON r.snap_id = p.snap_id AND r.instance_number = p.instance_number
    )
    SELECT
      ...
      -- Active Sessions
      ROUND(MIN(avg_active_sessions), 2) as min_active_sessions,
      ROUND(AVG(avg_active_sessions), 2) as avg_active_sessions_val,
      ROUND(MAX(avg_active_sessions), 2) as max_active_sessions,
      ROUND(PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY avg_active_sessions), 2) as p50_active_sessions,
      ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY avg_active_sessions), 2) as p95_active_sessions,
    ...
    DBMS_OUTPUT.PUT_LINE('  "min": ' || fmt_num(NVL(r.min_active_sessions, 0), 2) || ',');
    DBMS_OUTPUT.PUT_LINE('  "avg": ' || fmt_num(NVL(r.avg_active_sessions_val, 0), 2) || ',');
    DBMS_OUTPUT.PUT_LINE('  "max": ' || fmt_num(NVL(r.max_active_sessions, 0), 2) || ',');
    DBMS_OUTPUT.PUT_LINE('  "p50": ' || fmt_num(NVL(r.p50_active_sessions, 0), 2) || ',');
    DBMS_OUTPUT.PUT_LINE('  "p95": ' || fmt_num(NVL(r.p95_active_sessions, 0), 2));
    DBMS_OUTPUT.PUT_LINE('},');
  ...
  -- =========================================================================
  -- Uses the same CTE pipeline as performanceSummary above but returns
  -- individual rows rather than aggregates. The CTEs are duplicated here
  -- because Oracle SQL does not support sharing CTEs across statements;
  -- however, AWR data is typically cached after the first query so this
  -- second pass hits the buffer cache rather than re-reading from disk.
  -- =========================================================================
  ...
  FOR r IN (
    ...
    delta_data AS (
      SELECT
        snap_id, instance_number, begin_interval_time, end_interval_time, stat_name,
        current_value - prev_value as delta_value
      FROM awr_data
    ...
    SELECT
      a.snap_id,
      a.instance_number,
      TO_CHAR(SYS_EXTRACT_UTC(a.begin_interval_time), 'YYYY-MM-DD"T"HH24:MI:SS"Z"') as begin_time,
      TO_CHAR(SYS_EXTRACT_UTC(a.end_interval_time), 'YYYY-MM-DD"T"HH24:MI:SS"Z"') as end_time,
      ROUND(a.interval_seconds) as interval_seconds,
      ROUND(a.read_io_requests / a.interval_seconds) as read_iops,
      ROUND(a.write_io_requests / a.interval_seconds) as write_iops,
      ROUND(a.read_bytes / a.interval_seconds / 1024 / 1024, 2) as read_mbps,
      ROUND(a.write_bytes / a.interval_seconds / 1024 / 1024, 2) as write_mbps,
      ROUND(a.db_time_seconds / a.interval_seconds, 2) as active_sessions,
      ROUND(NVL(p.peak_read_iops, 0)) as peak_read_iops,
      ROUND(NVL(p.peak_write_iops, 0)) as peak_write_iops,
      ROUND(NVL(p.peak_read_mbps, 0), 2) as peak_read_mbps,
      ROUND(NVL(p.peak_write_mbps, 0), 2) as peak_write_mbps,
      ROUND(NVL(p.peak_active_sessions, 0), 2) as peak_active_sessions
    FROM avg_metrics a
    LEFT JOIN peak_metrics p ON a.snap_id = p.snap_id AND a.instance_number = p.instance_number
    ...
    DBMS_OUTPUT.PUT('"activeSessions": ' || fmt_num(NVL(r.active_sessions, 0), 2) || ', ');
    -- Peak values
    ...
    DBMS_OUTPUT.PUT('"peakActiveSessions": ' || fmt_num(NVL(r.peak_active_sessions, 0), 2));
    DBMS_OUTPUT.PUT('}');
  END LOOP;
...
SPOOL OFF
SET TERMOUT ON
PROMPT || AWR data collection complete (hybrid format). Output file: &file_name
