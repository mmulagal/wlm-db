...
import {
    DropDownCustomeItem,
    DsBaseComponentProps,
    MonitorPosition,
    TriggerType
} from '@netapp/design-system/dist/v2/types/types';
import { DsFlashingDotsLoader, DsPopover, DsTypography } from '@netapp/design-system';
import { DsButton, DsButtonProps } from '@netapp/design-system/dist/v2/components/dsButton/dsButton';
import useChangePosition, { PositionOffset } from '@netapp/design-system/dist/v2/hooks/usePosition';
...
export type SearchMethod =
    | 'basic'
    | 'smart'
    | ((items: DsDropDownListItemProps[], searchValue: string) => DsDropDownListItemProps[]);
...
const DsDropDownListContainer = forwardRef<HTMLDivElement, DsDropDownListContainerProps>(
    (
        { boundariesComponent, dropdownlist, onClickChild, isDisabled, disabledReason }: DsDropDownListContainerProps,
        ref
    ) => {
                ...
                <div ref={parentRef}>
                    ...
                    >
                        <div
                            ...
                            ref={clickOutsideRef}
                            className={_Classes('parentContainer', { isDisabled })}
                        >
                            {boundariesComponent}
                            <svg
    ...
    (
        ...
        useEffect(() => {
            ...
            const manageOpenDirection = (actionList: HTMLDivElement, boundaries: HTMLDivElement) => {
                ...
                if (top < 0) {
                    ...
                    totalHeight =
                        (actionList.clientHeight + parentHeight + (isAsubMenu ? margin * -1 : margin * 2)) * -1;
                    setPosition('top');
            ...
            if (childItems && childItems.length > 0) {
                return (
                    <DsDropDownListContainer
                        ref={childRef}
                        boundariesComponent={
                            <DsTypography variant="Regular_14" className="parentTitle">
                                {customeLabel(item)}
                            </DsTypography>
                        }
                        dropdownlist={{
                            ...
                            offsetX: offset,
                            offsetY: `${
                                boundariesRef.current ? boundariesRef.current.clientHeight * -1 - margin : 0
                            }px`,
                            trigger: 'click',
                            formatOptionLabel
                        }}
        ...
        return (
            <div
                ...
                ref={actionListRef}
                style={{ minWidth: boundariesRef.current?.clientWidth, ...style }}
                {...rest}
            >
                <div className="actionListRef" ref={ref} tabIndex={0} onMouseOver={onMouseOver}>
                    {isListExpanded && !isDisabled && (
                        <div className={`actionListContainer ${position}`} ref={clickOutsideRef}>
                            {searchMethod && (
                                <div className={`searchableContainer ${searchMethod.addItem ? 'searchAdd' : ''}`}>
