...
export const createUserDbPayload = (newUserDb: any) => {
    let payload = {
        ...
        dataVolumeSize: newUserDb?.newUserDataSizeUnit === 'TiB' ? newUserDb?.newUserDataSize * 1024 : newUserDb?.newUserDataSize,
        dataDrive: newUserDb?.driveLetter,
        logFileName: newUserDb?.newUserLogFileName,
        logVolumeSize: newUserDb?.newUserLogFileSizeUnit === 'TiB' ? newUserDb?.newUserLogFileSize * 1024 : newUserDb?.newUserLogFileSize,
