...
const ViewErrorInvestigation = ({ tableData }: { tableData: any }) => {
    const dispatch = useDispatch();
    const { selectedViewInvestigationRow } = useAppSelector(state => state.agenticAI);
    ...
    const tableProps = useTable({
        ...
        defaultSelectedRows: selectedViewInvestigationRow ? [selectedViewInvestigationRow.id] : [firstEnabledRow()],
        isSorting: false,
    ...
    useEffect(() => {
        ...
        if (rowNumber && rowNumber !== '0') {
            ...
            if (row) {
                dispatch(setSelectedViewErrorInvestigationRow(row));
            }
        }
    ...
    return (
        <div className={styles.categoryDialogContent}>
            <DsTypography variant="Regular_14">
                {t('databases.dashboard.view-error-investigation-dialog-text')}
            </DsTypography>
...
export default ViewErrorInvestigation;
