...
const LicenseTable = ({ lastColDetails, handleBulkAction }: StorageTierTableProps) => {
    ...
    const updatedTableData = useMemo(() => {
        if (inProgressOptimizationData?.[ASSESSMENT_CONFIG_NAMES.LICENSE]?.length) {
    ...
    const TableColDefs: ColumnProps[] = [
        ...
        lastColDetails(ASSESSMENT_CONFIG_NAMES.LICENSE, {}, inProgressOptimizationData, inProgressHostData)
    ];
    ...
    useEffect(() => {
        ...
        if (rowsData.length > 0 && inProgressOptimizationData?.[ASSESSMENT_CONFIG_NAMES.LICENSE]?.length) {
            checkBoxHandle(tableProps.selectionState, rowsData, disptach);
        }
    }, [tableProps.selectionState, inProgressOptimizationData]);

    const handleBulkOperation = () => {
        handleBulkAction(ASSESSMENT_CONFIG_NAMES.LICENSE, selectedRowsForOptimize);
    };
    return (
