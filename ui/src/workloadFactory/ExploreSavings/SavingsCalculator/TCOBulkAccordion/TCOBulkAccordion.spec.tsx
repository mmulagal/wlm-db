...
vi.mock('@netapp/design-system', () => ({
    AccordionCardContent: ({ children, className }: any) => <div data-testid="accordion-content" className={className}>{children}</div>,
    ...
    SelectField: ({ label, value, options, onChange, isDisabled, isLoading, variant }: any) => (
        <div data-testid={`select-${label?.replace(/\s+/g, '-')}`} data-disabled={String(!!isDisabled)} data-loading={String(!!isLoading)}>
            <button data-testid={`select-btn-${label?.replace(/\s+/g, '-')}`} onClick={() => onChange?.(options?.[0])}>change</button>
        </div>
    ),
...
vi.mock('@tlveng/wlm-ds', () => ({
    DsButton: ({ children, onClick, isDisabled, type }: any) => (
        <button data-testid={`ds-button-${children}`} onClick={onClick} disabled={isDisabled}>{children}</button>
    )
}));
...
vi.mock('../../../../common/Dialog/DialogComponent', () => ({
    default: ({ header, content, primaryButton, callback, closeCallback }: any) => (
        <div data-testid="dialog">{header}{content}</div>
    )
}));
...
vi.mock('./TCOAddHostTable/TCOAddHostTable', () => ({
    default: ({ onExploreSavings, onHandlerReady, onAuthRequired }: any) => (
        <div data-testid="tco-add-host-table">
            <button data-testid="add-host-callback" onClick={onExploreSavings}>add</button>
        </div>
    )
}));
...
vi.mock('./TCOBulkAccordion.module.scss', () => ({
    default: { tcoBulkAccordion: 'tcoBulkAccordion', header: 'header', accordionScrollContainer: 'accordionScrollContainer', centerValue: 'centerValue', centerText: 'centerText', rightWidgetButton: 'rightWidgetButton', accordionContent: 'accordionContent', ssdContainer: 'ssdContainer', setWidth: 'setWidth' }
}));

vi.mock('./HostInstanceSelection.module.scss', () => ({
    default: { hostInstanceSelection: 'hostInstanceSelection', fieldsContainer: 'fieldsContainer', fieldWrapper: 'fieldWrapper', instanceTypeContainer: 'instanceTypeContainer', errorContainer: 'errorContainer' }
}));
...
const makeStore = (overrides: any = {}) => {
    ...
    return configureStore({ reducer: { exploreSavingsBulk: bulkSlice.reducer, exploreSavings: esSlice.reducer, auth: authSlice.reducer } });
};
...
describe('TCOBulkAccordion', () => {
    ...
    it('renders accordion controller', () => {
        render(<Provider store={makeStore()}><TCOBulkAccordion /></Provider>);
        expect(screen.getByTestId('accordion-controller')).toBeTruthy();
    });

    it('renders header with selected hosts count', () => {
        const { container } = render(<Provider store={makeStore()}><TCOBulkAccordion /></Provider>);
    ...
    it('disables remove button when only 1 host', () => {
        ...
        render(<Provider store={store}><TCOBulkAccordion /></Provider>);
    ...
    it('shows 0 volumes when ebsResourceInfo is missing', () => {
        const store = makeStore({
            bulk: {
                selectedRowsForExploreSavingsEBSBulk: [
                    { id: 'host-1', name: 'Host 1', totalInstance: 1 }
                ]
            }
        });
        const { container } = render(<Provider store={store}><TCOBulkAccordion /></Provider>);
