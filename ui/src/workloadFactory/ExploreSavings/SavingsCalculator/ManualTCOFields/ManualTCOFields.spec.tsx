...
vi.mock('./ManualTCOFields.module.scss', () => ({
    default: {
        manualTCOFields: 'manualTCOFields',
        firstContainer: 'firstContainer',
        firstRow: 'firstRow',
        secondRow: 'secondRow',
        widthRegionSet: 'widthRegionSet',
...
vi.mock('../../../../store/workloadFactory/exploreSavingsSlice', () => ({
    ...
    setSelectedDeploymentModelForManualTCO: (val: any) => ({
        type: 'test/setSelectedDeploymentModelForManualTCO',
        payload: val
    }),
    ...
    const slice = createSlice({
        ...
        initialState: {
            ...
            getManualRegionsList: {
                manualRegionsData: { regions: [{ regionCode: 'us-east-1', regionName: 'US East' }] },
...
describe('ManualTCOFields', () => {
    ...
    it('shows error for cloned copies > 10', () => {
        render(
            <Provider store={makeStore({ numberOfClonedCopies: 15 })}>
                <ManualTCOFields printState={false} />
            </Provider>
        );
        expect(screen.getByTestId('error-Number-of-cloned-copies')?.textContent).toBe(
            'Number of cloned copies should be less than or equals to 10'
        );
    });

    it('shows error for monthly change rate > 100', () => {
        render(
            <Provider store={makeStore({ monthlyChangeRate: 150 })}>
                <ManualTCOFields printState={false} />
            </Provider>
        );
        expect(screen.getByTestId('error-Monthly-data-change-rate-(%)').textContent).toBe(
            'Monthly change rate should be less than or equals to 100'
        );
    });
    ...
    it('auto-selects region on mount when selectedManualRegion is null', () => {
        ...
        expect(dispatchSpy).toHaveBeenCalledWith(
            expect.objectContaining({ type: 'test/setSelectedRegionFromManualTCO' })
        );
    });
    ...
    it('auto-selects server edition on mount when null', () => {
        ...
        expect(dispatchSpy).toHaveBeenCalledWith(
            expect.objectContaining({ type: 'test/setSelectedManualServerEdition' })
        );
    });
    ...
    it('auto-selects deployment model on mount when null', () => {
        ...
        expect(dispatchSpy).toHaveBeenCalledWith(
            expect.objectContaining({ type: 'test/setSelectedDeploymentModelForManualTCO' })
        );
    });
