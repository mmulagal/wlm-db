...
vi.mock('../../../../ui-components/Typography', () => ({
    Text: ({ children, color, level, style }: any) => (
        <span data-color={color} data-level={level}>
            {children}
        </span>
    )
}));
...
describe('WindowFileServer', () => {
    ...
    it('shows NOT_AVAILABLE when no deployment type found', () => {
        const { container } = render(
            <Provider store={makeStore({ selectedHostDetails: {} })}>
                <WindowFileServer />
            </Provider>
        );
    ...
    it('shows loader when viewCalculationsLoading is true', () => {
        render(
            <Provider store={makeStore({ viewCalculationsLoading: true })}>
                <WindowFileServer />
            </Provider>
        );
    ...
    it('does not show loader when viewCalculationsLoading is false', () => {
        render(
            <Provider store={makeStore({ viewCalculationsLoading: false })}>
                <WindowFileServer />
            </Provider>
        );
