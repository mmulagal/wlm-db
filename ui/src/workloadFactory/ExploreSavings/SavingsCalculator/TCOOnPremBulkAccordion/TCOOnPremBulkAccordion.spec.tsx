...
vi.mock('@tlveng/wlm-ds', () => ({
    DsButton: ({ children, onClick, isDisabled, type }: any) => (
        <button data-testid={`ds-btn-${typeof children === 'string' ? children.replace(/\s+/g, '-') : 'btn'}`} onClick={onClick} disabled={isDisabled}>{children}</button>
    )
}));
...
vi.mock('../../../../common/Dialog/DialogComponent', () => ({
    default: ({ header, content }: any) => <div data-testid="dialog">{header}{content}</div>
}));
...
vi.mock('./TCOOnPremBulkAccordion.module.scss', () => ({
    default: { tcoOnPremBulkAccordion: 'tcoOnPremBulkAccordion', header: 'header', accordionScrollContainer: 'accordionScrollContainer', centerValue: 'centerValue', centerText: 'centerText', rightWidgetButton: 'rightWidgetButton', accordionContent: 'accordionContent', ssdContainer: 'ssdContainer', iconWrapper: 'iconWrapper', setWidth: 'setWidth' }
}));
...
describe('TCOOnPremBulkAccordion', () => {
    ...
    it('renders accordion controller', () => {
        render(<Provider store={makeStore()}><TCOOnPremBulkAccordion /></Provider>);
        expect(screen.getByTestId('accordion-controller')).toBeTruthy();
    });

    it('renders header with selected hosts count', () => {
        const { container } = render(<Provider store={makeStore()}><TCOOnPremBulkAccordion /></Provider>);
    ...
    it('disables add hosts when onPremiseDataLoading', () => {
        ...
        render(<Provider store={store}><TCOOnPremBulkAccordion /></Provider>);
    ...
    it('disables remove when only 1 host remains', () => {
        const store = makeStore({
            bulk: {
                selectedRowsForExploreSavingsOnPremBulk: [
                    { resourceId: 'r1', resourceName: 'OnPrem Host 1', deploymentModel: 'Standalone', sqlServerInstances: [], onPremisesNodes: [] }
                ]
            }
        });
        render(<Provider store={store}><TCOOnPremBulkAccordion /></Provider>);
    ...
    it('shows SSD tier card when ebsCapacity < 800 and hosts < 5', () => {
        ...
        const { container } = render(<Provider store={store}><TCOOnPremBulkAccordion /></Provider>);
    ...
    it('resolves host from onPremiseData items when host is a string', () => {
        const store = makeStore({
            ...
            es: {
                onPremiseData: { items: [{ resourceName: 'HostString', deploymentModel: 'Standalone', sqlServerInstances: [] }] }
            }
        });
        const { container } = render(<Provider store={store}><TCOOnPremBulkAccordion /></Provider>);
