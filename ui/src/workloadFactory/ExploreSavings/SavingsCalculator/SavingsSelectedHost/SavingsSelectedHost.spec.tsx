...
vi.mock('@netapp/design-system', () => ({
    DsTypography: ({ children, variant, className, title, style }: any) => <span data-variant={variant} className={className} title={title}>{children}</span>,
    FlashingDotsLoader: () => <div data-testid="loader" />
}));

vi.mock('./SavingsSelectedHost.module.scss', () => ({
    default: { selectedHosts: 'selectedHosts', valueArea: 'valueArea', container: 'container', numberContainer: 'numberContainer', value: 'value', heading: 'heading', heading2: 'heading2', disabledHeading: 'disabledHeading', disabledContent: 'disabledContent', loader: 'loader' }
}));
...
const makeStore = (overrides: any = {}) => {
    const slice = createSlice({
        name: 'exploreSavings',
        initialState: {
            selectedHostDetails: { name: 'Host1', totalInstance: 3, ebsResourceInfo: [{ id: 1 }, { id: 2 }], loading: false },
...
describe('SavingsSelectedHost', () => {
    ...
    it('renders Selected host label', () => {
        const { container } = render(<Provider store={makeStore()}><SavingsSelectedHost /></Provider>);
    ...
    it('shows NOT_AVAILABLE when no hostname', () => {
        const { container } = render(<Provider store={makeStore({ selectedHostDetails: { name: '', loading: false } })}><SavingsSelectedHost /></Provider>);
    ...
    it('adds partner volumes to total', () => {
        ...
        })}><SavingsSelectedHost /></Provider>);
        expect(container.textContent).toContain('3'); // 2 + 1
    });

    it('does not show volumes section for non-EBS mode', () => {
        const { container } = render(<Provider store={makeStore({ savingsCalculatorFrom: SAVINGS_CALC_MODE.AUTO_FSXW })}><SavingsSelectedHost /></Provider>);
    ...
    it('shows loaders when loading', () => {
        render(<Provider store={makeStore({ selectedHostDetails: { loading: true } })}><SavingsSelectedHost /></Provider>);
    ...
    it('handles OnPrem mode with host prop', () => {
        ...
        })}><SavingsSelectedHost host={{ resourceName: 'BulkHost', totalInstance: 2 }} /></Provider>);
    ...
    it('adjusts width for OnPrem tab', () => {
        const { container } = render(<Provider store={makeStore({ selectedExploreSavingsTab: WLF_TABS.MSSQL_ON_PREMISES })}><SavingsSelectedHost /></Provider>);
