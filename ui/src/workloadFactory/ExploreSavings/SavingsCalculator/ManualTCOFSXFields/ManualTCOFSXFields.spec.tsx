...
vi.mock('./ManualTCOFSXFields.module.scss', () => ({
    default: { manualTCOFSX: 'manualTCOFSX', fieldContainer: 'fieldContainer', rowContainer: 'rowContainer', deploymentModelWidth: 'deploymentModelWidth', storageCapacityField: 'storageCapacityField', SCWidth: 'SCWidth', SCUnitWidth: 'SCUnitWidth' }
}));
...
vi.mock('../../../../store/workloadFactory/exploreSavingsSlice', () => ({
    ...
    setSelectedManualStorageCapacityUnit: (val: any) => ({ type: 'test/setSelectedManualStorageCapacityUnit', payload: val }),
...
describe('ManualTCOFSXFields', () => {
    ...
    it('renders FSx for Windows File Server settings heading', () => {
        const { container } = render(<Provider store={makeStore()}><ManualTCOFSXFields printState={false} /></Provider>);
        expect(container.textContent).toContain('FSx for Windows File Server settings');
    });

    it('renders Deployment type select', () => {
        render(<Provider store={makeStore()}><ManualTCOFSXFields printState={false} /></Provider>);
    ...
    it('renders mockInputClone when printState is true', () => {
        const { container } = render(<Provider store={makeStore()}><ManualTCOFSXFields printState={true} /></Provider>);
    ...
    it('does not render TextFields when printState is true', () => {
        render(<Provider store={makeStore()}><ManualTCOFSXFields printState={true} /></Provider>);
    ...
    it('shows storage capacity error when > 64 TiB', () => {
        ...
        render(<Provider store={store}><ManualTCOFSXFields printState={false} /></Provider>);
        const input = screen.getByLabelText('Total storage capacity');
        fireEvent.change(input, { target: { value: '100' } });
        expect(screen.getByTestId('error-Total-storage-capacity')?.textContent).toBe('Maximum capacity value is 64 TiB');
    });
    ...
    it('auto-selects storage type on mount when null', () => {
        ...
        const dispatchSpy = vi.spyOn(store, 'dispatch');
        render(<Provider store={store}><ManualTCOFSXFields printState={false} /></Provider>);
        expect(dispatchSpy).toHaveBeenCalledWith(expect.objectContaining({ type: 'test/setSelectedManualStorageType' }));
    });
    ...
    it('auto-selects storage capacity unit on mount when null', () => {
        ...
        const dispatchSpy = vi.spyOn(store, 'dispatch');
        render(<Provider store={store}><ManualTCOFSXFields printState={false} /></Provider>);
        expect(dispatchSpy).toHaveBeenCalledWith(expect.objectContaining({ type: 'test/setSelectedManualStorageCapacityUnit' }));
    });
