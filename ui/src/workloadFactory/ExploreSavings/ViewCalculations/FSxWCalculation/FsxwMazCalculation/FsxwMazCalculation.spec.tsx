...
import FsxwMazCalculation from './FsxwMazCalculation';
...
const mockViewCalculationForFsxw = vi.fn().mockReturnValue({
    FSxWCalculation: [
        { label: 'FSxW storage cost', value: '$200' },
...
const defaultResponse = {
    fsxwCalculation: { totalMonthlyCost: '400' }
};
...
describe('FsxwMazCalculation', () => {
    ...
    it('should render AccordionCard with Multi AZ title', () => {
        renderComponent();
        expect(screen.getByTestId('accordion-title').textContent).toBe(GENERAL.ES_FSXW_MULTI);
    });
    ...
    it('should call viewCalculationForFsxw with type Multi and correct args', () => {
        renderComponent({ selectedDeploymentModel: 'FCI' });
        expect(mockViewCalculationForFsxw).toHaveBeenCalledWith({ ...defaultResponse, type: 'Multi' }, 'FCI');
    });
});
