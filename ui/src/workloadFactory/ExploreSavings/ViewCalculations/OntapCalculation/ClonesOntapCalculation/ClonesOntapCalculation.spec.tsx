...
import ClonesOntapCalculation from './ClonesOntapCalculation';
...
const createMockStore = (overrides: Record<string, any> = {}) => {
    const defaults = {
        viewCalculationsResponse: { fsxCloneCalculation: { totalCloneMonthlyCost: '100' } },
...
describe('ClonesOntapCalculation', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    it('should show cost in header when viewCalculationsResponse exists', () => {
        renderComponent({ viewCalculationsResponse: { fsxCloneCalculation: { totalCloneMonthlyCost: '250' } } });
    ...
    it('should call viewCalculation with correct arguments', () => {
        const response = { fsxCloneCalculation: { totalCloneMonthlyCost: '100' } };
        renderComponent({ viewCalculationsResponse: response, selectedDeploymentModel: 'FCI' });
        expect(mockViewCalculation).toHaveBeenCalledWith(response, 'FCI');
    });
});
