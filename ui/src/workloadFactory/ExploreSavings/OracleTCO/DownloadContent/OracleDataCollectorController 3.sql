...
DECLARE
  ...
  -- Pre-flight flags (set by blocking checks)
  v_preflight_aborted BOOLEAN := FALSE;
  v_awr_view_accessible BOOLEAN := TRUE;
  v_sp_view_accessible  BOOLEAN := TRUE;
BEGIN
  ...
  BEGIN
    ...
    IF v_is_cdb = 'YES' THEN
      ...
      EXCEPTION WHEN OTHERS THEN
        v_pdb_count := 0;
        DBMS_OUTPUT.PUT_LINE('|| WARNING: Could not query v$pdbs (ORA' || SQLCODE || ').');
        DBMS_OUTPUT.PUT_LINE('|| Grant SELECT_CATALOG_ROLE or SELECT ANY DICTIONARY to the collection user.');
      END;
      ...
      IF v_pdb_count = 0 THEN
        DBMS_OUTPUT.PUT_LINE('|| WARNING: No PDBs found. The collection user may lack privileges on v$pdbs.');
      END IF;
  ...
  -- =========================================================================
  -- PRE-FLIGHT: Test PL/SQL dynamic SQL access to critical views
  -- Role-based grants (e.g. SELECT_CATALOG_ROLE) may NOT work inside PL/SQL.
  -- Only direct system privileges (SELECT ANY DICTIONARY) or direct object
  -- grants are effective in PL/SQL dynamic SQL.
  -- =========================================================================
  DECLARE
    v_test         NUMBER;
    v_pflight_warn BOOLEAN := FALSE;
    v_pflight_fatal BOOLEAN := FALSE;
    v_container_clause VARCHAR2(100) := '';
    v_dg_count     NUMBER := 0;
    v_standby_cnt  NUMBER := 0;
  BEGIN
    IF INSTR(UPPER(v_current_user), 'C##') = 1 THEN
      v_container_clause := ' CONTAINER=ALL';
    END IF;

    -- Test v$pdbs (CDB environments)
    IF v_is_cdb = 'YES' AND v_pdb_count = 0 THEN
      BEGIN
        EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM v$pdbs WHERE con_id > 2' INTO v_test;
        IF v_test = 0 THEN
          v_pflight_warn := TRUE;
          DBMS_OUTPUT.PUT_LINE('|| ');
          DBMS_OUTPUT.PUT_LINE('|| PRE-FLIGHT WARNING: v$pdbs returns 0 PDBs in PL/SQL context.');
          DBMS_OUTPUT.PUT_LINE('||   This CDB likely has PDBs that are not visible to this user.');
          DBMS_OUTPUT.PUT_LINE('||   The pdbList in the output JSON will be empty.');
          DBMS_OUTPUT.PUT_LINE('||   FIX (step 1): GRANT SELECT ANY DICTIONARY TO ' || v_current_user || v_container_clause || ';');
          DBMS_OUTPUT.PUT_LINE('||   FIX (step 2): ALTER USER ' || v_current_user || ' SET CONTAINER_DATA = ALL CONTAINER = CURRENT;');
          DBMS_OUTPUT.PUT_LINE('||     OR: GRANT SELECT ON sys.v_$pdbs TO ' || v_current_user || v_container_clause || ';');
        END IF;
      EXCEPTION WHEN OTHERS THEN
        v_pflight_warn := TRUE;
        DBMS_OUTPUT.PUT_LINE('|| ');
        DBMS_OUTPUT.PUT_LINE('|| PRE-FLIGHT WARNING: Cannot access v$pdbs (ORA' || SQLCODE || ').');
        DBMS_OUTPUT.PUT_LINE('||   FIX (step 1): GRANT SELECT ANY DICTIONARY TO ' || v_current_user || v_container_clause || ';');
        DBMS_OUTPUT.PUT_LINE('||   FIX (step 2): ALTER USER ' || v_current_user || ' SET CONTAINER_DATA = ALL CONTAINER = CURRENT;');
      END;
    END IF;

    -- Test v$archive_dest_status (DataGuard standby info)
    BEGIN
      EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM v$dataguard_config' INTO v_dg_count;
    EXCEPTION WHEN OTHERS THEN
      v_dg_count := 0;
    END;

    IF v_dg_count > 1 THEN
      BEGIN
        EXECUTE IMMEDIATE
          ...
          || ' AND type != ''LOCAL'''
          INTO v_standby_cnt;
        IF v_standby_cnt = 0 THEN
          v_pflight_warn := TRUE;
          DBMS_OUTPUT.PUT_LINE('|| ');
          DBMS_OUTPUT.PUT_LINE('|| PRE-FLIGHT WARNING: DataGuard is enabled but v$archive_dest_status');
          DBMS_OUTPUT.PUT_LINE('||   returns 0 standby destinations in PL/SQL context.');
          DBMS_OUTPUT.PUT_LINE('||   The standbyDatabases array in the output JSON will be empty.');
          DBMS_OUTPUT.PUT_LINE('||   FIX (step 1): GRANT SELECT ANY DICTIONARY TO ' || v_current_user || v_container_clause || ';');
          DBMS_OUTPUT.PUT_LINE('||   FIX (step 2): ALTER USER ' || v_current_user || ' SET CONTAINER_DATA = ALL CONTAINER = CURRENT;');
          DBMS_OUTPUT.PUT_LINE('||     OR: GRANT SELECT ON sys.v_$archive_dest_status TO ' || v_current_user || v_container_clause || ';');
          DBMS_OUTPUT.PUT_LINE('||   NOTE: If DataGuard is enabled but no standby destinations are configured,');
          DBMS_OUTPUT.PUT_LINE('||     this warning is expected and the standbyDatabases array will be empty.');
        END IF;
      EXCEPTION WHEN OTHERS THEN
        v_pflight_warn := TRUE;
        DBMS_OUTPUT.PUT_LINE('|| ');
        DBMS_OUTPUT.PUT_LINE('|| PRE-FLIGHT WARNING: Cannot access v$archive_dest_status (ORA' || SQLCODE || ').');
        DBMS_OUTPUT.PUT_LINE('||   FIX (step 1): GRANT SELECT ANY DICTIONARY TO ' || v_current_user || v_container_clause || ';');
        DBMS_OUTPUT.PUT_LINE('||   FIX (step 2): ALTER USER ' || v_current_user || ' SET CONTAINER_DATA = ALL CONTAINER = CURRENT;');
      END;
    END IF;

    -- Test core V$ views (v$parameter as representative) -- BLOCKING
    BEGIN
      EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM v$parameter WHERE ROWNUM = 1' INTO v_test;
    EXCEPTION WHEN OTHERS THEN
      v_pflight_fatal := TRUE;
      DBMS_OUTPUT.PUT_LINE('|| ');
      DBMS_OUTPUT.PUT_LINE('|| FATAL: Cannot access v$parameter in PL/SQL (ORA' || SQLCODE || ').');
      DBMS_OUTPUT.PUT_LINE('||   vCPUs, SGA, and PGA data cannot be collected.');
      DBMS_OUTPUT.PUT_LINE('||   These are required for TCO analysis. Collection ABORTED.');
      DBMS_OUTPUT.PUT_LINE('||   FIX: GRANT SELECT ANY DICTIONARY TO ' || v_current_user || v_container_clause || ';');
    END;

    -- Test DBA_ dictionary views (dba_data_files as representative) -- BLOCKING
    BEGIN
      EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM dba_data_files WHERE ROWNUM = 1' INTO v_test;
    EXCEPTION WHEN OTHERS THEN
      v_pflight_fatal := TRUE;
      DBMS_OUTPUT.PUT_LINE('|| ');
      DBMS_OUTPUT.PUT_LINE('|| FATAL: Cannot access dba_data_files in PL/SQL (ORA' || SQLCODE || ').');
      DBMS_OUTPUT.PUT_LINE('||   Storage info (totalDatabaseSizeGB) cannot be collected.');
      DBMS_OUTPUT.PUT_LINE('||   This is required for TCO analysis. Collection ABORTED.');
      DBMS_OUTPUT.PUT_LINE('||   FIX: GRANT SELECT ANY DICTIONARY TO ' || v_current_user || v_container_clause || ';');
    END;

    -- Test AWR views (dba_hist_sysmetric_summary as representative) -- checked later in decision logic
    BEGIN
      EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM dba_hist_sysmetric_summary WHERE ROWNUM = 1' INTO v_test;
    EXCEPTION WHEN OTHERS THEN
      v_awr_view_accessible := FALSE;
    END;

    -- Test Statspack tables (stats$sysstat as representative) -- checked later in decision logic
    BEGIN
      EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM stats$sysstat WHERE ROWNUM = 1' INTO v_test;
    EXCEPTION WHEN OTHERS THEN
      v_sp_view_accessible := FALSE;
    END;

    IF v_pflight_fatal THEN
      DBMS_OUTPUT.PUT_LINE('|| ');
      DBMS_OUTPUT.PUT_LINE('|| ============================================================================');
      DBMS_OUTPUT.PUT_LINE('||                 PRE-FLIGHT CHECK FAILED - COLLECTION ABORTED');
      DBMS_OUTPUT.PUT_LINE('|| ============================================================================');
      DBMS_OUTPUT.PUT_LINE('|| ');
      DBMS_OUTPUT.PUT_LINE('|| One or more critical views required for TCO analysis are inaccessible.');
      DBMS_OUTPUT.PUT_LINE('|| ');
      DBMS_OUTPUT.PUT_LINE('|| REQUIRED FIX (run as SYSDBA):');
      DBMS_OUTPUT.PUT_LINE('||   GRANT SELECT ANY DICTIONARY TO ' || v_current_user || v_container_clause || ';');
      IF v_is_cdb = 'YES' THEN
        DBMS_OUTPUT.PUT_LINE('||   ALTER USER ' || v_current_user || ' SET CONTAINER_DATA = ALL CONTAINER = CURRENT;');
      END IF;
      DBMS_OUTPUT.PUT_LINE('|| ');
      DBMS_OUTPUT.PUT_LINE('|| After granting privileges, re-run the data collector.');
      DBMS_OUTPUT.PUT_LINE('|| No data has been collected.');
      v_preflight_aborted := TRUE;
    ELSIF v_pflight_warn THEN
      DBMS_OUTPUT.PUT_LINE('|| ');
      DBMS_OUTPUT.PUT_LINE('|| NOTE: SELECT_CATALOG_ROLE grants work at the SQL prompt but may NOT');
      DBMS_OUTPUT.PUT_LINE('||   be effective inside PL/SQL. Use SELECT ANY DICTIONARY (a system');
      DBMS_OUTPUT.PUT_LINE('||   privilege) or direct object grants instead.');
      DBMS_OUTPUT.PUT_LINE('||   For CDB environments, also run as SYSDBA:');
      DBMS_OUTPUT.PUT_LINE('||     ALTER USER ' || v_current_user || ' SET CONTAINER_DATA = ALL CONTAINER = CURRENT;');
      DBMS_OUTPUT.PUT_LINE('||   Collection will proceed, but some data may be incomplete.');
    END IF;
  END;
  ...
  -- Skip if pre-flight blocking checks already failed
  IF v_preflight_aborted THEN
    :v_script_action := 'NONE';
  ELSIF INSTR(UPPER(v_edition), 'ENTERPRISE EDITION') > 0 THEN
    IF UPPER(v_pack_access) IN ('DIAGNOSTIC', 'DIAGNOSTIC+TUNING') THEN
      IF v_awr_snaps >= v_min_snaps AND v_awr_time_span >= v_min_hours THEN
        IF NOT v_awr_view_accessible THEN
          DBMS_OUTPUT.PUT_LINE('|| ');
          DBMS_OUTPUT.PUT_LINE('|| ============================================================================');
          DBMS_OUTPUT.PUT_LINE('||           AWR PERFORMANCE VIEWS INACCESSIBLE');
          DBMS_OUTPUT.PUT_LINE('|| ============================================================================');
          DBMS_OUTPUT.PUT_LINE('|| ');
          DBMS_OUTPUT.PUT_LINE('|| WARNING: Cannot access dba_hist_sysmetric_summary in PL/SQL context.');
          DBMS_OUTPUT.PUT_LINE('||   AWR snapshots exist but performance data views are inaccessible.');
          DBMS_OUTPUT.PUT_LINE('||   Attempting STATSPACK fallback...');
          DBMS_OUTPUT.PUT_LINE('|| ');
          -- Attempt Statspack fallback on Enterprise Edition
          IF v_sp_snaps >= v_min_snaps AND v_sp_time_span >= v_min_hours AND v_sp_view_accessible THEN
            :v_script_action := 'STATSPACK';
            DBMS_OUTPUT.PUT_LINE('|| FALLBACK: Using STATSPACK instead of AWR.');
            DBMS_OUTPUT.PUT_LINE('|| WARNING: AWR is preferred for Enterprise Edition. Fix AWR access for best results:');
            DBMS_OUTPUT.PUT_LINE('||   GRANT SELECT ANY DICTIONARY TO ' || v_current_user || ';');
          ...
          ELSE
            :v_script_action := 'NONE';
            DBMS_OUTPUT.PUT_LINE('|| FATAL: STATSPACK fallback also unavailable.');
            IF NOT v_sp_view_accessible THEN
              DBMS_OUTPUT.PUT_LINE('||   stats$sysstat is also inaccessible.');
            ELSIF v_sp_snaps < v_min_snaps THEN
              DBMS_OUTPUT.PUT_LINE('||   STATSPACK has insufficient snapshots (' || v_sp_snaps || ').');
            ELSE
              DBMS_OUTPUT.PUT_LINE('||   STATSPACK not installed or no data available.');
            END IF;
            DBMS_OUTPUT.PUT_LINE('||   FIX: GRANT SELECT ANY DICTIONARY TO ' || v_current_user || ';');
            DBMS_OUTPUT.PUT_LINE('||   Collection ABORTED. No performance data collected.');
            DBMS_OUTPUT.PUT_LINE('|| ');
          END IF;
        ELSE
          :v_script_action := 'AWR';
    ...
    ELSE
      ...
      IF v_sp_snaps >= v_min_snaps AND v_sp_time_span >= v_min_hours THEN
        IF NOT v_sp_view_accessible THEN
          ...
          DBMS_OUTPUT.PUT_LINE('|| ');
          DBMS_OUTPUT.PUT_LINE('|| ============================================================================');
          DBMS_OUTPUT.PUT_LINE('||       STATSPACK PERFORMANCE TABLES INACCESSIBLE - COLLECTION ABORTED');
          DBMS_OUTPUT.PUT_LINE('|| ============================================================================');
          DBMS_OUTPUT.PUT_LINE('|| ');
          DBMS_OUTPUT.PUT_LINE('|| FATAL: Cannot access stats$sysstat in PL/SQL context.');
          DBMS_OUTPUT.PUT_LINE('||   Statspack snapshots exist but performance data tables are inaccessible.');
          DBMS_OUTPUT.PUT_LINE('||   performanceSnapshots and performanceSummary cannot be collected.');
          DBMS_OUTPUT.PUT_LINE('||   FIX: GRANT SELECT ON perfstat.stats$sysstat TO ' || v_current_user || ';');
          DBMS_OUTPUT.PUT_LINE('|| ');
        ELSE
          :v_script_action := 'STATSPACK';
