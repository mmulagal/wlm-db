    ...
    reducers: {
        setResourceTables: (state, action: PayloadAction<any>) => {
            ...
            action.payload.map((value: any) => {
                const tableRow = state.tables.find(
                    table => table.databaseName === value.databaseName && table.tableName === value.tableName
                );
                if (!tableRow) {
...
export const { setResourceTables, resetMssqlTables } = resourceSlice.actions;
