...
router.post(
    `${BASE_URL}/v1/credentials/:credentialsId/regions/:region/instances/:instanceId/storage-savings`,
    async (req: {}, res: any) => {
        setTimeout(() => {
...
router.post(`${BASE_URL}/v1/regions/:region/manual-storage-savings`, async (req: {}, res: any) => {
    setTimeout(() => {
        generateResponse(res, 200, StorageSavings);
    }, 5000);
});
