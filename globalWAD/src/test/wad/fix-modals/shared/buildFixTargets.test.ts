...
import { buildFileSystemFixTargets, buildVolumeFixTargets } from '@wad/fix-modals/shared/buildFixTargets';
import { createMockResource } from '@test/helpers/testUtils';

describe('buildFixTargets', () => {
    describe('buildVolumeFixTargets', () => {
        it('maps selected resources using resource id as fix id', () => {
            const resources = [
                createMockResource('vol-1', { subConfig: 'dedupe' }),
                createMockResource('vol-2')
            ];
