...
import { createTagsColumn } from '../shared/tagsColumn';
...
export const FileSystemResourcesTable = ({ wadApi, tableScope }: FileSystemResourcesTableProps) => {
    ...
    const [tagsResource, setTagsResource] = useState<ResourceScanRecord | null>(null);
    const handleViewTags = useCallback((resource: ResourceScanRecord) => setTagsResource(resource), []);
    const handleCloseTags = useCallback(() => setTagsResource(null), []);
    ...
    const columns = useMemo<ReadonlyArray<TableColumn<ResourceScanRecord>>>(() => {
        ...
        const tagsColumn = createTagsColumn({ onView: handleViewTags });
        ...
        return [...spliceExtras(baseColumns, [], ResourceColumnId.LAST_ANALYZED), tagsColumn, ctaColumn];
    }, [
        configuration.columns,
        configuration.extraColumns,
        tableScope,
    ...
    return (
        <EnrichmentProvider pendingIds={new Set()}>
            <WadResourcesTable
                wadApi={wadApi}
            ...
            <ResourceTagsDialog resource={tagsResource} onClose={handleCloseTags} />
        </EnrichmentProvider>
    );
};
